...
public class DiggerBuilder_Room : MonoBehaviour
{
	...
	{
		[Tooltip("Delay between looping build + generate (-1 for non looping)")]
		public float loop;
	...
	public void BeginDig() {ClearStructure(false); DiggerAlgorithm.i.Dig(diggerConfig);}

	void OnEnable(){diggerConfig.generateComplete += AssembleStructure;}
	void OnDisable() {diggerConfig.generateComplete -= AssembleStructure;}
	...
	{
		//Begin invoke to loop another dig if needed
		if(builder.loop >= 0) Invoke("BeginDig", builder.loop);
...
#region Floor
	...
	public void ClearStructure(bool renew)
	{
		...
		if(diggerConfig.isGenerating) return;
