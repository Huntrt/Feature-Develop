...
#region Generation
	...
	/// Dig repeatedly until meet the required amount
	public void Dig(DiggerConfig config, bool overwrite = false)
	{
		//Stop if already dig except when need to overwriten
		if(config.isGenerating) {if(!overwrite) {return;}}
	...
	IEnumerator Digging(DiggerConfig config, DigPlot miner, DigPlot digged)
	{
		...
		//Checking progress at this miner to continue digging
		CheckingDigProgress(config, miner);
		//Dig are complete when there no miner left
		if(config.miners.Count <= 0) {CompleteDig(config);}
	}

	//? Try to bypassing or digging in any 4 direction
	void DecideDirectionToDig(DiggerConfig config, DigPlot miner)
	{
		/// DIG
		//If there still available direction
		if(miner.availableDirection.Count > 0) 
		{
			//Create an temporary copy of miner's available direction
		...
		if(miner.availableDirection.Count <= 0 && config.miners.Count <= 1 )
		{
			//Will continuous run until miner has direction to bypass
			while (miner.bypassedDirection == -1)
			{
				//Randomly choose any direction to decide to bypass
	...
	bool[] RandomizingDigDirection(DiggerConfig config)
	{
		...
		for (int r = 0; r < result.Length; r++)
		{
			...
			//If each direction has different priority
			if(config.digChance.useDirectionalPriority)
			{
			...
			{
				//Result are true since it alway need to dig atleast once
				//? Which direction to dig are already randomly choose 
				if(config.miningConstraint.minimum > 0) {result[r] = true; continue;}
				//Decide continue dig by using global chance to see result direction to dig
				if(config.digChance.global >= decide) {result[r] = true;}
			}
		}
		return result;
	}
	...
	void TryToDig(DiggerConfig config, DigPlot miner, int dir, bool allow)
	{
		...
		//STOP dig if has reach the limit or not allow to dig
		if(config.dugs.Count >= config.limit || !allow) {return;}
	...
	DigPlot GetAdjacentDug(DiggerConfig config, DigPlot dug, int dir, out Vector2 nextCoord)
	{
		...
		return Digger.FindDugAtCoordinate(config.dugs, nextCoord);
	}
	...
	{
		//Does this miner need to retry
		bool retry = false;
		//Retry when haven't reached the limit but still need to mine the bare minimum while not count as bypass
		if(config.dugs.Count < config.limit && miner.digCount < config.miningConstraint.minimum && miner.bypassedDirection == -1) retry = true;
		//Retry again at this miner if needed
		if(retry) StartCoroutine(Digging(config, miner, miner));
	...
	void SetAllDugNeighbors(DiggerConfig config)
	{
		...
		for (int d = 0; d < config.dugs.Count; d++) for (int n = 0; n < 4; n++)
		{
			...
			DigPlot findDug = Digger.FindDugAtCoordinate(config.dugs, neighbor.coordinate);
			//If found an dug
			if(findDug != null)
			{
	...
	void CompleteDig(DiggerConfig config)
	{
		...
		//Call complete dig
		config.generateComplete?.Invoke();
...
#region Preview
	void Previewing(DiggerConfig config, DigPlot miner, DigPlot digged)
	{
		...
		PreviewConfig.PreviewObj newPre = new PreviewConfig.PreviewObj();
